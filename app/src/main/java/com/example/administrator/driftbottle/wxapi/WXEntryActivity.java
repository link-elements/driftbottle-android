...
public class WXEntryActivity extends Activity implements IWXAPIEventHandler{
	...
	private static final String AppSecret = "1869dbafc38f00988560ef7b3d37e8bb";
			...
			switch (tag) {
				case NetworkUtil.GET_TOKEN: {
					...
					try {
						...
						Log.d("微信获取 WXEntry - code", "openId:" + openId + ", accessToken:" + accessToken + ", refreshToken:" + refreshToken + ", scope:" + scope);
	...
	public void onReq(BaseReq req) {
		Log.d("微信 - WXEntry", "onReq.type: " + req.getType());
		switch (req.getType()) {
	...
	public void onResp(BaseResp resp) {
		Log.d("微信 - WXEntry", "onResp.type:" + resp.getType() + ", onResp.errCode:" + resp.errCode);
		...
		if (resp.getType() == ConstantsAPI.COMMAND_SENDAUTH) {
			...
			NetworkUtil.sendWxAPI(handler, String.format("https://api.weixin.qq.com/sns/oauth2/access_token?" +
							"appid=%s&secret=%s&code=%s&grant_type=authorization_code", APP_ID,
					AppSecret, code), NetworkUtil.GET_TOKEN);
		}
        finish();
	}
